namespace DiveMap.Core
{
    ...
    public static class ModeRules
    {
        ...
        /// <summary>
        /// A→B ตรง ๆ ไม่ได้ แต่ A→View→B ได้ไหม.
        ///
        /// 🔴 21 ส.ค. 2026 — user (build 9016): "เข้า AR แล้วไปเข้าโหมดโดรน ขยับอะไรไม่ได้เลย
        /// ลองออกไปเข้าโหมด preview ก็ขยับไม่ได้ แถมยังแสดงหน้าโดรนอีก"
        ///
        /// <see cref="CanEnter"/> ปฏิเสธ Ar→Tour โดยตั้งใจ (AR กับ Edit เข้าจากหน้าแมพเท่านั้น)
        /// และตอนที่กฎนี้ถูกเขียน โหมดทุกโหมดเข้าจากเมนูของ Unity เอง ซึ่งอยู่ในหน้าแมพเสมอ
        /// จึงไม่มีทางเรียกข้ามได้จริง. **WO-PIVOT เปลี่ยนข้อนั้นไปแล้ว**: เจ้าบ้าน (แอป RN) สั่ง
        /// โหมดเข้ามาตรง ๆ จากหมุดบนแผนที่ โดยที่ Unity ตัวเดิมยังมีชีวิตอยู่ ⇒ คำสั่ง "โดรน"
        /// ขณะอยู่ใน AR = <c>Request</c> คืน false เงียบ ๆ · โหมดค้างที่ Ar · ผู้เล่นเห็น chrome
        /// ของโดรนที่ฝั่งแอปวาดไว้ แต่ไม่มีทั้งริกโคจร (AllowsOrbit(Ar)=false) และไม่มีทัวร์
        /// ⇒ "ขยับอะไรไม่ได้เลย" ตามที่รายงานเป๊ะ
        ///
        /// ทางแก้เป็นการ **เดินผ่านฮับ** ไม่ใช่การเปิดเส้นทางใหม่: กฎว่า "AR ต้องเข้าจากหน้าแมพ"
        /// ยังอยู่ครบ ระบบแค่ออกจาก AR ให้เรียบร้อยก่อน (ArSession.Restore ได้ทำงานจริง) แล้วค่อย
        /// เข้าโหมดที่ขอ — ตรงกับเจตนาเดิมที่ว่า "ไม่มีสถานะไหนที่ผู้ใช้ติดอยู่ได้"
        /// </summary>
        public static bool RoutesViaHub(AppMode from, AppMode to)
        {
            if (from == to || CanEnter(from, to)) return false;
            if (to == AppMode.View) return false;             // ออกไปฮับได้อยู่แล้ว
            return CanEnter(from, AppMode.View) && CanEnter(AppMode.View, to);
        }
    }
}
