...
namespace DiveMap.Core
{
    ...
    public static class FleeMath
    {
        ...
        /// <summary>
        /// ระยะเตือนสูงสุด (ตอนพุ่งเต็มสปีด) วัดเป็นช่วงตัวปลา นับจากระยะชนออกไป.
        ///
        /// 🔴 21 ส.ค. 2026 — user: "ผมอยากให้ผู้ใช้งานสนุกกับการเล่น เจอสัตว์ เห็นสัตว์ …
        /// ระยะอื่น ๆ ผมว่าควรให้ใกล้กว่านี้"
        ///
        /// นี่คือค่าที่ตัดสินว่าเกมนี้สนุกหรือน่าหงุดหงิด ไม่ใช่ค่าความสมจริง: ฝูงที่แตกตั้งแต่ยัง
        /// มองไม่ทันเห็น = ผู้เล่นได้ดำน้ำในทะเลว่างเปล่า. เลิกอ้างอิง <see cref="DiverPanicRadius"/>
        /// (สูตรของเว็บ 0.85R+6L) ตรงนี้โดยตั้งใจ — มันกว้างกว่าระยะชนหลายเมตรในฝูงกลาง
        /// (R=12 → 21.6 ม. เทียบระยะชน 13.9 ม.) และแคบกว่าระยะชนในฝูงใหญ่ ซึ่งผิดทั้งสองทาง.
        /// สูตรเว็บยังอยู่ครบในฟังก์ชันเดิมเพื่อการเทียบเคียง เพียงแต่ไม่ใช่คนตัดสินระยะอีกต่อไป.
        /// </summary>
        /// ⬇️ ประวัติการจูน (user ตัดสิน — ห้ามดันกลับขึ้นโดยไม่ถาม). ตัวเลขในวงเล็บคือระยะเตือน
        ///    ตอนพุ่งเต็มสปีดของฝูงตัวอย่าง (รัศมี 12 u ปลายาว 1.9 u = ฝูงรัศมี 2 ม. ปลา 32 ซม.):
        ///      สูตรเว็บ 0.85R+6L (+128 ซม.) → 2.0 ช่วงตัว (+63 ซม.) → 0.5 (+16) → **กลับมา 2.0**
        ///
        /// 🔴 บทเรียนของรอบ 0.5: user สั่ง "ลดอีก ยังมากไป" จากตารางที่ผมติดหน่วยผิดเป็น "เมตร"
        ///    ทั้งที่เป็นหน่วยโลก (<see cref="ItemPicker.UnitsPerMetre"/> = 6 หน่วย/เมตร) —
        ///    เขาเห็นเลข 14.85 แล้วเข้าใจว่าฝูงตกใจตั้งแต่ 15 เมตร ทั้งที่ของจริงคือ 2.5 เมตร
        ///    พอเห็นเลขเป็นเมตรจริงก็เลือกกลับมาที่ 2.0 ทันที
        ///    ⇒ **ตัวเลขที่ส่งให้ user ตัดสิน ต้องแปลงเป็นเมตรเสมอ** ค่าคงที่ในไฟล์นี้เป็นหน่วยโลก
        public const double StartleWarnBodyLengths = 2.0;

        /// <summary>
        /// ระยะที่ฝูงเริ่มรู้สึกถึงโดรน — โตตามความเร็ว.
        ///
        /// ที่ความเร็วเกณฑ์หรือช้ากว่า = <see cref="ContactRadius"/> พอดี ⇒ ลอยนิ่ง/คลานเข้าไป
        /// **ฝูงไม่ตกใจเลยจนกว่าจะถึงตัว** (user 21 ส.ค.) · ที่คันเร่งเต็ม = ระยะชน +
        /// <see cref="StartleWarnBodyLengths"/> ช่วงตัว ซึ่งเป็นระยะเตือนที่แคบพอให้ผู้เล่น
        /// ได้เห็นฝูงก่อนมันจะแตก.
        /// </summary>
        public static double StartleRadius(double spreadR, double fishLen, double diverSpeed)
        {
            ...
            double far = near + Math.Max(fishLen, 0.5) * StartleWarnBodyLengths;
