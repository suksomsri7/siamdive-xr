...
namespace DiveMap.Runtime.Ui
{
    ...
    public sealed class UiShell : MonoBehaviour
    {
        ...
        private void ApplySafeArea(bool force)
        {
            ...
            // 🔴 ในแอปรวม: ไม่ใช้ Screen.safeArea เลย (WO-PIVOT, 16 ส.ค. 2026)
            //
            // ประวัติของบั๊กนี้สองรอบ — รอบแรกผมอ่านภาพผิดด้านและแก้ผิดทาง จึงจดไว้ให้ครบ:
            //   • user ชี้ว่า "แมพแรกคือตำแหน่งที่ถูก แมพที่สองเป็นต้นไปผิด"
            //   • ในภาพของแมพที่สอง ปุ่มถูกหดเข้ามาจากขอบ **ซ้ายและขวาเท่ากันทั้งสองข้าง**
            //     ~220 px ซึ่งไม่ใช่รูปทรงของรอยบากจริง (iPhone แนวนอนมีรอยบากข้างเดียว)
            //     ⇒ ค่าที่ได้มาไม่ใช่พื้นที่ปลอดภัยของจอนี้ แต่เป็นก้อนที่คำนวณจากสภาพอื่น
            //
            // สาเหตุเชิงโครงสร้าง: ในโหมดฝังตัว **เจ้าบ้านเป็นคนถือ view และเป็นคนหมุนจอ** Unity
            // จึงได้ค่าที่ไม่ตรงกับสิ่งที่มันกำลังวาดอยู่จริง และค่านั้นเปลี่ยนไปมาระหว่างการเข้า-ออก
            // แมพ ⇒ ปุ่มขยับเองโดยที่ไม่มีอะไรในโค้ดเราสั่ง
            //
            // ทางแก้ที่ไม่ต้องเดา: ฝั่งเจ้าบ้าน (RN) จัดการขอบปลอดภัยของมันเองอยู่แล้ว และ Unity
            // ได้ view เต็มจอมา ⇒ ใช้เต็มจอ แล้วให้ปุ่มเว้นระยะขอบของตัวเอง (14 css px ที่มีอยู่)
            // ซึ่งตรงกับ "ตำแหน่งที่ถูกต้อง" ในภาพแรกของ user พอดี
            //
            // บิลด์เดี่ยวไม่แตะ: ที่นั่น Unity เป็นเจ้าของจอจริง ค่า safeArea เชื่อถือได้ และรอยบาก
            // จะบังปุ่มจริงถ้าไม่เว้น
            if (NativeBridge.EmbeddedInHost)
            {
                var full = new Rect(0f, 0f, screen.x, screen.y);
                if (!force && full == _appliedSafeArea && screen == _appliedScreen) return;
                _appliedSafeArea = full;
