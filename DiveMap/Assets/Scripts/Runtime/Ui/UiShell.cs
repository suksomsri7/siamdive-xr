...
namespace DiveMap.Runtime.Ui
{
    ...
    public sealed class UiShell : MonoBehaviour
    {
        ...
        private void ApplySafeArea(bool force)
        {
            ...
            // 🔴 พื้นที่ปลอดภัยที่ "ค้างเป็นค่าแนวตั้ง" ตอนอยู่ในแอปเจ้าบ้าน (WO-PIVOT, 15 ส.ค. 2026)
            //
            // อาการที่ user เจอ: เข้าแมพแรกแล้วปุ่มทั้งชุดอยู่ต่ำกว่าปกติ ~90 px (วัดจากภาพหน้าจอ
            // ของ user เอง) พอเข้าแมพที่สองเป็นต้นไปกลับมาถูกตำแหน่ง
            //
            // ราก: ในแอปรวม **เจ้าบ้านเป็นคนหมุนจอ** (RN ล็อกแนวนอนตอนเข้าทัวร์) ไม่ใช่ Unity —
            // และ Unity ห้ามแตะ Screen.orientation เองในโหมดนี้ ⇒ ช่วงแรก Screen.width/height
            // สลับเป็นแนวนอนแล้ว แต่ Screen.safeArea ยังเป็นก้อนของแนวตั้ง (รอยบากอยู่ "ด้านบน"
            // สูงเท่ารอยบากจริง) ค่านั้นถูกต้องเมื่อมองเป็นแนวตั้ง แต่ผิดทันทีที่จอเป็นแนวนอน
            //
            // ตรวจได้จากตัวมันเอง: จอแนวนอนของ iPhone ไม่มีรอยบากด้านบน — มีแต่ด้านซ้าย/ขวา
            // ⇒ ถ้ากว้างกว่าสูง แต่ยังมีขอบบน/ล่างหนากว่า ~3% ของความสูง แปลว่ากำลังถือค่าเก่า
            // ⇒ ข้ามไปก่อน แล้วรออ่านใหม่เฟรมถัดไป (ApplySafeArea ถูกเรียกทุกเฟรมอยู่แล้ว)
            //
            // ไม่แก้ด้วยการ "หน่วงตอนสร้าง HUD" เพราะเราไม่รู้ว่าเจ้าบ้านจะหมุนเสร็จเมื่อไร และ
            // การเดาเวลาคือสิ่งที่ทำให้บั๊กชนิดนี้กลับมาเป็นรอบที่สอง
            if (!force && screen.x > screen.y)
            {
                float top = screen.y - sa.yMax;
                float bottom = sa.yMin;
                if (Mathf.Max(top, bottom) > screen.y * 0.03f) return;
            }
