...
namespace DiveMap.Runtime
{
    ...
    public static class QcMapShot
    {
                                       ...
                                       System.Action<bool> onDone)
        {
            ...
            //
            // 🔴 THE SIGNATURES HERE ARE AppBoot's, NOT THE ONES THIS FILE SHIPPED WITH. The
            // version of this pass written on 3 Aug called Configure(waterLevel, centre, radius),
            // an overload that belonged to the fog rework that was reverted with the rest of that
            // day's rendering work (HANDOFF §10). Restoring the call as written would have
            // dragged WaterFog.RangeAt back in with it — i.e. would have changed the picture in
            // order to photograph it. This calls exactly what AppBoot.OnBuilt calls, so the frame
            // this pass measures is the frame the player gets, which is the entire point of it.
            EnvMode.Reset();
            DepthAtmosphere.Configure(result.WaterLevel);
            ...
            for (int attempt = 0; attempt < 2; attempt++)
            {
                ...
                // 🔴 READ OFF RenderSettings, NOT off a helper. The original wrote
                // DepthAtmosphere.CurrentRange() and WaterFog.FactorAt(), both of which were part
                // of the reverted fog rework and neither of which exists on this baseline.
                // RenderSettings is the better source anyway: it is what the SHADER samples, so a
                // range logged from here cannot disagree with the frame the way a re-derivation
                // can. LinearFogFactor is the built-in linear-fog equation, and it computes a
                // number for the log — it writes nothing.
                ...
                Debug.Log($"[QCMap] {shortId} '{result.MapName}' posed={(retry ? "diver-retry" : "diver")} " +
                          ...
                          $"nearRim={LinearFogFactor(Mathf.Max(0f, camToContent - result.Radius), fogStart, fogEnd) * 100f:F0}% " +
                          $"farRim={LinearFogFactor(camToContent + result.Radius, fogStart, fogEnd) * 100f:F0}% " +
        ...
        /// <summary>
        /// How much of a surface at <paramref name="distance"/> is replaced by fog colour, 0..1 —
        /// the built-in LINEAR fog equation, which is the mode this app ships
        /// (<c>AppBoot.SetupLighting</c> sets <c>FogMode.Linear</c>).
        ///
        /// 🔎 It is a pure function of three numbers and it writes nothing. Present so a log line
        /// can say "the far rim is 80% fog" instead of printing two distances and leaving a reader
        /// to do the arithmetic — the same job <c>WaterFog.FactorAt</c> did before that file's
        /// version of it was reverted.
        /// </summary>
        private static float LinearFogFactor(float distance, float start, float end)
        {
