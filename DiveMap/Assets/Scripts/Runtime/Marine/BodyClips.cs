...
namespace DiveMap.Runtime.Marine
{
    ...
    public sealed class BodyClips
    {
        ...
        /// <summary>
        /// 🔴 THE GATE, applied. Rip every <see cref="Animation"/> component out of a GLB that is
        /// not allowed to have one — see <see cref="ClipPlay.MayAnimate"/> for why this is not
        /// optional.
        ///
        /// Called the moment a model lands and BEFORE anything else touches it, because
        /// <c>playAutomatically</c> is true by default and glTFast has already assigned
        /// <c>animation.clip</c>: leave the component in place for even one frame of an active
        /// object and a statue takes a step. <c>DestroyImmediate</c> rather than
        /// <see cref="Object.Destroy"/> for exactly that reason — <c>Destroy</c> defers to the end
        /// of the frame, which is one frame too late — and it is safe here because these are
        /// runtime scene objects, never assets.
        ///
        /// Returns how many it removed, for the caller's log line. Zero is the normal answer and
        /// it is not a failure: most of the library ships no clips at all.
        /// </summary>
        public static int Strip(GameObject root)
        {
            if (root == null) return 0;
            Animation[] found = root.GetComponentsInChildren<Animation>(true);
            if (found == null || found.Length == 0) return 0;

            int n = 0;
            for (int i = 0; i < found.Length; i++)
            {
                if (found[i] == null) continue;
                // Stop first: the component may already have started its clip, and destroying it
                // does not put the bones back where they were.
                found[i].Stop();
