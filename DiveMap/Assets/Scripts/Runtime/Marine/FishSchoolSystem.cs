        ...
        private void Update()
        {
            ...
            for (int si = 0; si < _render.Count; si++)
            {
                ...
                for (int k = 0; k < sr.Count; k++)
                {
                    ...
                    // (ครึ่งชีวิต ~0.45 วิ — user ยืนยันข้อ 2 'ส่ายหัวถี่' 8 ส.ค.) เหมือนสัตว์จริงที่คอมันหน่วงการสะบัดของลำตัวเอง.
                    if (sr.DrawRot != null && k < sr.DrawRot.Length)
                    {
                        if (!sr.DrawRotInit) sr.DrawRot[k] = rot;
                        float kSm = 1f - Mathf.Exp(-dt / 0.45f);   // 0.12 เบาไป (ลดแกว่ง 2Hz ~45% ตาไม่เห็น) — 0.45 ลด ~5 เท่า
