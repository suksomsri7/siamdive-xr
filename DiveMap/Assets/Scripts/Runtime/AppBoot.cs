...
namespace DiveMap.Runtime
{
    ...
    public sealed class AppBoot : MonoBehaviour
    {
        ...
        public void SwitchMapFromHost(string shortId)
        {
            ...
            // 🔴 แมพใหม่ต้องเริ่มจาก "โหมดดูแมพ" เสมอ (16 ส.ค. 2026)
            //
            // Unity อยู่ในหน่วยความจำข้ามการเข้า-ออกจอ (ดูหมายเหตุด้านบน) ⇒ โหมดที่ค้างจากแมพ
            // ก่อนหน้าจะตามมาด้วย ถ้าโหมดนั้นคือ AR หรือทัวร์ ริกของมันยัง "ยืม" กล้องหลักไปอยู่
            // (AR ถึงขั้นเอากล้องไปแขวนใต้ XROrigin ที่ถูกสเกล) แล้วฉากใหม่จะถูกสร้างใต้กล้องที่
            // ไม่ได้อยู่ที่เดิม · และเมื่อเจ้าบ้านสั่งโหมดเดิมซ้ำ ModeManager จะเห็นว่า "อยู่โหมด
            // นั้นอยู่แล้ว" จึงไม่ยิงเหตุการณ์เปลี่ยนโหมด — โค้ดที่ตั้งค่าโหมดจึงไม่ทำงานเลย
            //
            // อาการที่ user รายงาน: "AR แมพแรกไม่มีปัญหา พอแมพที่ 2 จอดำ"
            if (ModeManager.Instance != null && ModeManager.Current != AppMode.View)
            {
                Debug.Log($"[Native] leaving {ModeManager.Current} before loading {shortId}");
