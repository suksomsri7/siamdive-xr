...
>
> **สรุปตัวเลข: 18.5 / 86 ≈ 21% ของฟีเจอร์เว็บ** (แต่ถ้าวัดแค่ "ภาพนิ่งของฉาก" ≈ 85%)
...
## D. ทัวร์ดำน้ำ (โดรน) — 0/10 ❌ ทั้งหมด
`enterTour()` :3635 · `exitTour()` · `tourUpdate()`
| # | ฟีเจอร์ | หลักฐาน |
|---|---|---|
| D1 | เข้า/ออกโหมดทัวร์ + ล็อกจอแนวนอน | `enterTour`, `tourLockLandscape()` |
| D2 | จอยสติ๊ก 2 ตัว (ขึ้น-ลง/เลี้ยว + เดินหน้า) | `makeStick()`, `stickL`, `stickR`, `knobL/R` |
| D3 | ไฟหน้าโดรน (โคนแสง + spotlight) | `_applyHeadlight()`, `lightBtn`, `mkBeam()` :3669 |
| D4 | อ่านค่าความลึกสดขณะบิน | `tourDepth`, `depthMetres()` |
| D5 | **ถ่ายรูป** | `tourShot`, `captureThumb()` |
| D6 | **อัดวิดีโอ + นาฬิกาจับเวลา** | `tourRec`, `stopRec()` :3801, `_recClock()` |
| D7 | บับเบิลจากโดรน | `droneBubble()` |
| D8 | ชนวัตถุแล้วถูกดันออก | `_ejectFromSolids()` |
