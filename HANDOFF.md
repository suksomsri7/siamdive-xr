...
## §8 — Session 2026-08-03 (user เล่น build 244 แล้วบอกว่า "แย่กว่าเดิม") — อ่านก่อนแตะการเรนเดอร์

### 🔴 บทเรียนใหญ่ที่สุดของ session นี้: **QC ถ่ายผิดรูปมาตลอด**
QC เดิมมี 2 อย่าง: ถ่าย 9 โมเดล**เดี่ยวๆ ในสตูดิโอ**ห่างแมพ 4,000 u กับถ่าย**แมพเดียว (Chang) จากมุม orbit**
⇒ **Atlantis / Posidon / Hanuman / Harddeep / T-13 / Tu-1 ไม่เคยเข้าเฟรม CI สักครั้ง** และ**มุมผู้เล่นไม่เคยถูกถ่ายบนแมพไหนเลย**
ผลคือตัวเลขดีขึ้นจริงในสตูดิโอ (สิงห์ `blackOfSubject` 20.21%→5.64%) แต่ user เปิดเกมแล้วยังดำ **เพราะของที่ดำในเกมไม่ใช่ 9 ตัวที่เราถ่าย**
→ `Runtime/QcMapShot.cs` (ใหม่): 7 แมพ · กล้องระดับตานักดำน้ำ (12 u เหนือพื้น ก้ม 6°) · **3 เฟรมต่อ pose** (ภาพจริง / ซ่อนแมพ / ปิด fog) ⇒ ส่วนต่าง = "แมพ" กับ "หมอก" **เป็นไบต์ ไม่ใช่ความเห็น**
→ เกณฑ์: `fogWork ≥ 0.40` · `backdropSpan ≥ 0.08` · Atlantis `dark% < 33.1` (วัดจากรูปที่ user ส่งมา)
📌 **กติกาใหม่: ห้ามประกาศว่าแก้เรื่องภาพแล้ว จนกว่าจะมีเฟรมจากแมพที่ user เล่นจริง**

### 🔑 หมอกหาย/ฉากไม่มีมิติ — **ไม่ใช่เพราะยกแสงพื้น** (ตัดด้วยเลขแล้ว)
`AmbientFloor` ที่ WO-E4 ใส่ไว้ **ไม่เคยแตะ sky/equator band เลย** (ต่ำกว่าค่าจริง 3-20 เท่า) ยกเฉพาะ ground band ที่กินผิวหันลง ⇒ ไม่ล้างหมอก ไม่ทำให้ทรายสว่าง **ไม่ต้อง revert `SeabedBounceGain`/`BandFloorMultiple`**
ตัวการจริง 3 ตัว:
1. **fog near/far = 500/9000 เป็นเลขของ "กล้องเว็บที่ลอยนอกแมพ 950 u"** แต่แอปพาผู้เล่น**เข้าไปข้างใน**แมพรัศมี 340 u → ของที่ระยะ 200 u ได้หมอก **0%** ขอบไกลได้ **6.5%** · เลขชุดเดียวกัน เรขาคณิตคนละเรื่อง — **ตรวจมาแค่ฝั่งเดียวมาตลอด**
   → `WaterFog.RangeAt(depth, R, camToContent)` แก้สมการจากข้อกำหนด "ขอบไกลของแมพที่เรายืน = น้ำ 80%" · floor อยู่**นอก**เทอมความลึก (ไม่งั้นลึกแล้ว reach สั้นกว่าตัวแมพ = บั๊กเดิม)
2. **สีหมอกมืดลง 5.2 เท่า** หลัง WO-E3 (`#123a55` × attenuation) → หมอกกลายเป็น "ตัวทำให้มืด" ไม่ใช่ "ตัวทำให้ขุ่น" → ฉายขอบไกลของแมพผ่าน matrix กล้องจริงแล้วอ่าน ramp ที่แถวนั้น (v=0.90 คือแถวของ**กล้องเว็บ** เท่านั้น)
3. **ramp ครึ่งเข้มไม่เคยขึ้นจอ** — ที่ระดับตานักดำน้ำ น้ำกินแค่ v∈[0,0.28] ของจอ → `backdropSpan` วัดจากรูป user ได้ **0.031** (ควร ≥0.08)

### ⚠️ ยังไม่แก้ (จงใจ — รอเฟรมจริงก่อน)
**ดวงอาทิตย์ไม่ถูกหรี่ตามความลึกเลย** (`sun.intensity 1.2` คงที่) ขณะที่ ambient/fog/backdrop คูณ `Attenuation` เหลือ 25-49% ที่ 61.8 ม.
⇒ **ยิ่งลึก วัตถุยิ่งสว่างเทียบกับน้ำ** = "ของคมชัด น้ำบาง ไม่มีมิติ" · ทราย 2.6× ของน้ำ → จะเป็น 1.27× ถ้าหรี่
**ขัดกับ invariant ที่ WaterFog เขียนไว้เอง** ("subject:background คงที่ทุกความลึก") เพราะ sun คือแสงส่วนใหญ่ของ subject แต่ไม่อยู่ในการคูณนั้น

### 🔑 texture: **เว็บ = master เป๊ะ และ XR ของเราสว่างกว่าเว็บทุกตัว** (วัด 8 โมเดล × 3 ทาง)
singha web 74.83 / master 75.24 / **XR 113.52** · chang 88.46 / 88.33 / **137.31** · kraken 155.03 / 154.72 / 154.76
KTX2 transfer function ถูกทุกไฟล์ทุก slot (BaseColor+Emissive=SRGB, Normal+MR=LINEAR) · ETC1S เราใช้ quality 255 สูงกว่าเว็บ
⇒ **ความมืดถูก Meshy อบมาใน master ตั้งแต่ต้น ไม่ใช่ pipeline เราทำเสีย** · ป้อนไฟล์เว็บเข้า pipeline **แย่กว่า** (guard "NEVER UPSCALE" → ได้ 512²/8k tris)
🔴 **แต่ pipeline เราทำเสียจริง 1 อย่าง และเป็น geometry:** `ruin_*` เว็บส่ง mesh เต็ม **101k tris** แต่ XR ลดเหลือ **35k** ตาม `LODS.hero.targetTris` (`/root/dive3d/optimize_xr.mjs:51`) → พื้นที่ที่ shading normal หันหนีแสง **0.80% → 3.45%** = ปื้นดำที่ไม่ได้มาจาก texture (ตรงกับรูป Atlantis ที่ user ถ่ายมา)

### 🔑 แกนคุณภาพใหม่ที่ไม่เคยวัด: "หยาบเกินขนาดที่วางจริง"
เกณฑ์จาก distribution จริง (729 instance / 61 โมดูล / 7 แมพ): **`tris/m² < 8` หรือ `texels/m² < 1,000`**
- **Tier 1 asset เสียจริง** (ต้นฉบับเว็บก็เท่ากัน ไม่ใช่ pipeline): `cc0:rock_b` **218 tris — สามเหลี่ยมกว้าง 6 เมตร** วางที่ Posidon×12/Hanuman×3 · `cc0:rock_e` 705 · `cc0:rock_cluster` 84 + **ไม่มี texture เลย**
- 🔴 **`cc0:rock_b` 12/15 instance มี scale ติดลบ** → determinant ลบ = winding/normal กลับด้าน (แก้ที่**ข้อมูลแมพ** ไม่ใช่ asset)
- **Tier 2 asset ดีแต่วางใหญ่เกินงบ** 12 ตัว → ทางแก้คือ **texture 4096²** ไม่ใช่ rebuild · หนักสุด `ruin:domed_temple` วางที่ **128 เมตร = 69 texels/m²** · `ruin:broken_pillars` ×18 ตัว

### 🔑 ครีบ "เร็ว" ทั้งที่ Hz ตรงเว็บ — **วัด Hz อย่างเดียวไม่พอ**
shader ของเว็บ (`builder.html:1507`) มี **4 ตัวเลข** แต่พอร์ตมาแค่ `wRate` อีก 3 ตัวยังมาจากตารางขนาดของสัตว์เดี่ยว
| | เว็บ | Unity เดิม | ผิด |
|---|---|---|---|
| beatHz barracuda | 0.796 | 0.796 | ✅ |
| **amp** | 3.96% ของลำตัว | 7.5% | **1.92×** |
| **cycles** | 0.267 คลื่น | 0.85 | **3.19×** |
| bank / pitch | **ไม่มีเลย** | 30° / ±29° | มีทั้งที่ไม่ควรมี |
**ความเร็วปลายหาง = amp × 2πf** ⇒ Hz เท่ากันแต่หางเร็วกว่า 1.92 เท่า · cycles 3.19 เท่า = อ่านเป็น "สั่น" ไม่ใช่ "ว่าย" · bank/pitch = เหตุที่ฝูงดูกระจัดกระจายคนละมุม
→ `Core/SchoolFormation.cs` (ใหม่): พอร์ต slot-formation 9 โหมดของเว็บ + speed law `min(cap, dist×chaseK)` (ถึง slot แล้วชะลอ — boids ความเร็วคงที่ไม่มีทางทำได้) + bait-ball · log `[School]` = `slotRms/inR/nn/pol` อ่านจาก log ได้ว่าเกาะกลุ่มจริงไหมโดยไม่ต้องดูภาพ
📌 **pod ยังใช้ boids โดยตั้งใจ** (เว็บก็แยก: `instanced = !pod`)

### 🐞 บั๊กที่ซ่อนตัวได้เพราะ "ทางที่ไม่เคยถูกเดิน"
1. **ACESInputMat แถว 2 ลอกผิดแถว** `0.13383` แทน `0.01566` → แถวรวม 1.118 → เขียวยืมน้ำเงิน 8.5 เท่า = **คราบเขียวทั้งเฟรม ยิ่งลึกยิ่งแรง** · เทส drift จับไม่ได้เพราะ**สำเนา C# กับ HLSL ผิดเหมือนกัน** → `RowsOfTheAcesMatricesSumToOne` (ตรวจเลขคณิต หลอกด้วยสำเนาผิดตรงกันไม่ได้)
2. **Android build พัง 3 สัปดาห์** — `}` ปิด namespace อยู่**ข้างใน `#if UNITY_ANDROID`** → iOS ข้ามบล็อกเลยเขียวตลอด · Android เกิน 1 ตัว `CS1022` · ไม่มีใครเห็นเพราะ Android build เฉพาะรอบยิงมือ → **`tools/csharp-check` ตอนนี้ parse 4 ชุด define** (default/ANDROID/IOS/EDITOR) และบอกแพลตฟอร์มใน error
3. **`com.unity.modules.animation` ไม่เคยอยู่ใน `Packages/manifest.json`** → โค้ด animation ทั้งก้อนของ glTFast อยู่ใต้ `#if UNITY_ANIMATION` เลยถูกคอมไพล์ทิ้ง = **คลิปไม่เคยถูกสร้างเลยแม้แต่ครั้งเดียว**
4. **`GoldFx.ApplyGold` เป็น no-op** — เซ็ต `_EmissionColor/_Metallic/_Glossiness` ซึ่งเป็นชื่อของ Unity Standard **ไม่มีอยู่ใน `glTF/PbrMetallicRoughness`** · log เดิมนับ material ที่ *เห็น* ไม่ใช่ที่ *แก้* จึงรอดมาได้ → ย้ายไป `Core/GoldShading.cs` + negative-control test ที่ pin ว่าชื่อเดิมไม่มีจริง

### 🔑 rig/animation — ข้อสมมติที่ผิด และความจริง
**ผิด**: "pipeline อบยุบ node ทำลาย rig" · **จริง**: XR build ถูกป้อน**ไฟล์คนละใบ** = master Meshy 100k tris ที่**ไม่เคย rig**  ส่วน rig+คลิปอยู่บน**ไฟล์เว็บ**เท่านั้น · เต่ารอดเพราะ worklist ชี้ raw ที่มี rig
→ `rig_transfer.mjs` ย้าย rig จากไฟล์เว็บมาใส่ master · **กับดัก: rig กับ master คนละ origin** ต้องเลื่อน**โครงกระดูก** (`IBM' = IBM·T(+Δ)`) ไม่ใช่เลื่อน mesh
→ 🔴 **Unity วัด "ตำแหน่ง bone origin" ไม่ใช่ตำแหน่ง vertex** ⇒ rig ที่ปีกเป็น **leaf หมุนอย่างเดียว** (กระเบน/ปลาแสงอาทิตย์/เพนกวิน **11 ตัว**) อ่านได้ `poseDelta = 0` ตลอดกาล **ต่อให้ปีกกระพือ 41°** → `add_tip_bones.mjs` (เมทริกซ์ deform เหมือนเดิมทางพีชคณิต รูปร่างไม่ขยับแม้แต่ทศนิยมที่ 4)
→ `QcAnimShot` วัด 2 ทางที่พังคนละแบบ: `poseDelta` (`Animation.Sample()` deterministic = ไฟล์/import) vs `liveDelta` 6 **เฟรม** (= แอป tick ไหม) · verdict เรียงตามคนที่ต้องแก้: `no-clips`→`no-skin`→`frozen-pose`→`frozen-live`→`moving`
→ ผลรวม: **66 โมเดล 132 ไฟล์ · สแกนหมวดทะเล 93 โมดูล → moving 74 · frozen 0**
⚠️ **ต้องสแกนกลุ่มที่ "ผ่านอยู่แล้ว" ด้วย** — `msh:manta` อยู่ในกลุ่มมีคลิปเดิม แต่เป็น leaf-wing เหมือนกัน เกือบรอดไปเป็นตัวนิ่งตัวเดียวในแมพ
⚠️ **rollback ต้องคืนไฟล์ขึ้น CDN ด้วย ไม่ใช่แค่ในเครื่อง** — เคยเหลือ xr0 ใหม่คู่ xr1 เก่าเสิร์ฟอยู่จริง = LOD คนละรุ่น

### 📌 กติกาที่เพิ่มใน session นี้
1. **เกณฑ์ที่ orchestrator ตั้งเอง ต้องยอมให้ executor หักล้างด้วยการวัด** — รอบนี้ตั้งผิด 3 ข้อ (p95 dRGB ≥30% ไม่มีโมเดลไหนทำได้แม้แต่ตัวที่ ship แล้ว · corridor y 0.45 เป็นหินตันจริง · `%ต่ำกว่า sRGB 45` ทับกันสนิทและทิศกลับกัน)
2. **contrast แบบค่าเฉลี่ยใช้ตัดสิน "เห็นชัดไหม" ไม่ได้** — บาราคูด้าที่ 52 ม. metric ฟ้อง `flat` แต่ตาเห็นชัดมาก เพราะตัวมันมีทั้งขาวและดำ ค่าเฉลี่ยเลยใกล้พื้นหลัง → ต้องวัด range ในตัว subject ด้วย
3. **user สั่ง: Android ยังไม่เอา iOS ให้ผ่านก่อน** → ยิง build ด้วย `players:false, ios:true`

## §9 WO-E5 — "ทุกวัตถุดำ": สิ่งที่ตัดทิ้งแล้ว และเหลืออะไร (3 ส.ค. 2026)

> 🔴 **อ่านก่อนตั้งสมมติฐานใหม่** ทุกบรรทัดข้างล่างมีตัวเลขรองรับจากเฟรมของ Unity หรือจากไฟล์จริง
> อย่าไล่ซ้ำ — ถ้าจะรื้อข้อไหน ต้องมีการวัดที่หักล้างตัวเลขที่อ้างไว้

### ตัดทิ้งแล้ว (พร้อมเลขที่ใช้ตัด)
| สมมติฐาน | เลขที่ใช้ตัด |
|---|---|
| texture มืด | grand_byzantine surface mean 130 · pctBelow45 0.57% แต่ยังดำ |
| UV gutter ซึมเข้า mip | 0.30% @2048² → 0.00% @64² (ลดลงตาม mip = ตรงข้ามบั๊กหมึกยักษ์) |
| normal map | `costOfNormalMap` = **0.00pp** บน 9 ใน 12 โมเดล · sweep normalScale 0.5/0.25/0 ไม่ขยับ |
| tangent ที่หายไป | `[QCPilot] tangent-added HTMS_Chang` (คู่ md5 สะอาดทั้ง 4 slot) = **no-effect −0.02pp** |
| ETC1S encoding | `[QCPilot] normalmap-uastc` = **no-effect +0.00pp** · และเว็บใช้ ETC1S เหมือนกันแต่แย่กว่า |
| metallic | metal channel เฉลี่ย 0.09-0.21/255 = dielectric |
| occlusion map | ไม่มีในไฟล์ไหนเลย ทั้งเว็บและ XR |
| COLOR_0 vertex colour | ไม่มีในไฟล์ไหนเลย |
| LOD1 | ใช้เฉพาะ FishAssetPick (ฝูงปลา) · ruin/decor โหลด LOD0 เสมอ |
| เงา | `noShadow` 26.75% → 26.42% |
| หมอก / ความลึก | โหมดกลางวัน return ก่อนโค้ดหรี่ทั้งหมด แต่ยังดำ |
| geometry ถูกหั่น | rebuild 101k แล้ว `pctDark` ลงเท่าเว็บทุกหลัก แต่ยังดำ |
| **ACES** | สแกน 37 จุด 1e-4..4.0 เทียบ three.js ที่พอร์ตสด → **ผลต่าง 0.000e+00 ทุกจุด** |

### เลขที่ชี้ว่าเหลืออะไร
ในโหมดกลางวัน ambient band = 0.53-0.72 · albedo ซุ้ม = 0.162 linear
⇒ มืดที่สุดที่เป็นไปได้ (ปิดดวงอาทิตย์ทิ้ง เหลือ ground band) = 0.099 scene-linear = **ไบต์ 89**
แต่ ladder ถ่ายได้ **26.75% ของโมเดลที่ไบต์ 0** ซึ่งต้องการ scene-linear < `ToneMap.BlackFloor` = 0.00186

> **แสงน้อยกว่าที่ ambient อย่างเดียวจะให้ได้ 50 เท่า** ⇒ ไม่ใช่ "มืดเกินไป" แต่คือ **ambient ไม่ถึงพิกเซลเหล่านั้นเลย**

เหลือทางเดียว: **เทอม ambient/GI ของ renderer ที่ glTFast สร้าง** — `QcRuinLadder.GiStateRungs`
ทดสอบ 3 ทาง (`lightProbeOff` · `lightmapIndexClear` · `flatAmbient0.8`) พร้อม log `[QCGi]` ที่เทียบ
กับ **quad อ้างอิง** ในเฟรมเดียวกัน (material เดียวกับพื้นทราย ไม่ได้มาจาก glTFast = control ที่ดีที่สุด)

### 🔴 กฎที่ได้มาแพง — ทุก rung ต้องวัดบน mask เดียวกับ baseline
rung ที่เปลี่ยนความสว่างทั้งเฟรม (ปิด tone curve, ปิดไฟ, เปลี่ยน ambient) ถ้าคำนวณ subject mask
ใหม่เอง จะได้ `subject=100%` = เฉลี่ยรวมน้ำกับพื้นหลังเข้าไปด้วย แล้วเอาไปเทียบกับ rung ที่เฉลี่ย
เฉพาะโมเดล = **เทียบคนละประชากร** · เคยหลุดไปถึง user เป็นตัวเลข "สว่างขึ้น 6.4 เท่า" ที่ไม่จริง
`QcPixels.SubjectMask` + `SceneLinearOfMask` + `RungMaskTests` ตรึงกฎนี้ไว้แล้ว

### บทเรียนอีกข้อ — สถิติที่คำนวณบนหน้าต่าง เป็นข้อความเกี่ยวกับหน้าต่างนั้น
ผมเคยวัด "พลังงานอิสระของ x/y ใน normal map" บนบล็อก 4×4 แล้วอ้างเป็นคุณสมบัติของทั้ง texture
(ได้ 0.01% สรุปว่า ETC1S ทำลาย normal map) · กวาดหน้าต่างจริงได้ 0.02 → 42.21% ตามขนาดหน้าต่าง
**ก่อนอ้างสถิติเป็นคุณสมบัติของอะไร ต้องกวาดพารามิเตอร์ก่อนแล้วดูว่าค่าคงที่ไหม**
(master คงที่ ~50% ทุกหน้าต่าง = อ้างได้ · ไฟล์ที่ ship ไม่คงที่ = อ้างไม่ได้)
