...
### ⚠️ ค้างอยู่ 1 ข้อ — ภาพ QC ยังพิสูจน์ C5 ไม่ได้ (โค้ดเสร็จ เทสผ่าน 38 ตัว)
`qc_ui_flee.png` ยังถ่ายได้แค่ฝูงปลาปกติ เพราะบน CI **ความเร็วโดรนที่วัดได้ตันอยู่ ~10.4 u/s** ขณะที่เกณฑ์ตกใจคือ >11
(log บอกครบแล้ว: `[Flee] school=3 panic=0.00 camSpeed=10.4/11 dist=58 R=93`)
- ระยะไม่ใช่ปัญหาแล้ว (58 < 93 = อยู่ในรัศมีตกใจ) เหลือแค่ความเร็ว
- ตั้งสมมติฐานไว้ให้: `DroneFlight.Step` มี inertia 0.09/เฟรม — 5 วิ บน CI ≈ 40 เฟรม ควรถึง ~29 u/s แล้ว
  → สงสัยว่าโดนหน่วงจาก seabed clamp / solid avoidance หรือ `Ry=-1` ไม่ได้เป็นคันเร่งเต็ม
  **ตรวจโดยพิมพ์ `_state.Vel` ใน `[Tour] frame=` log ก่อนแก้อะไร**
- บนมือถือจริง FPS สูงกว่ามาก ความเร็วถึง 30 u/s สบายๆ → คาดว่าฟีเจอร์ทำงานอยู่แล้ว แต่ **ยังไม่มีหลักฐาน**
