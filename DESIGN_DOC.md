...
3. `sculpt` → สร้าง mesh พื้นด้วย polar grid (rings×seg) เหมือน seabed เว็บ — ระวัง winding order (gotcha เดิมจาก headless test)
   - 🔴 **การเรียง index คนละแบบ:** เว็บ = `1 + (r-1)·seg + s` (index 0 = จุดกลาง) เรียงตามมุมพิกัดเว็บ · แอป = `(r-1)·seg + j` เรียงตามมุมพิกัด Unity → ต้องสะท้อน `j → (seg−j) % seg` และกลับเครื่องหมาย `areaSlopeZ` ที่ขอบ JSON (`SculptCoord`)
